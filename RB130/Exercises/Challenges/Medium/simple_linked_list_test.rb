...
class LinkedListTest < Minitest::Test
  ...
  def test_pop_1_item
    list = SimpleLinkedList.new
    list.push(1)
    assert_equal 1, list.pop # returns the head and removes it from the linked list, should also decrease the instance variable thats tracking the list size
    assert_equal 0, list.size
  end
  ...
  def test_from_a_empty_array
    list = SimpleLinkedList.from_a([]) # #from_a takes an argument, iterates through the argument and calls #push on each element
    assert_equal 0, list.size
    assert_nil list.peek
  end
